...
const MainMail = () => {
	...
	const handleCheckAll = () => {
	  ...
	  if (isChecked) {
		...
		mailCheckboxInputs.forEach((input:any) => {
		  input.checked = true;
		});
	  } else {
		mailListElements.forEach((element) => {
	...
	const [isMailCcVisible, setIsMailCcVisible] = useState(false);
	const [isMailBccVisible, setIsMailBccVisible] = useState(false);
	
	const handleCcBtnClick = () => {
	  setIsMailCcVisible(true);
	  setIsMailBccVisible(false);
	};
	
	const handleBccBtnClick = () => {
	  setIsMailBccVisible(true);
	  setIsMailCcVisible(false);
	};
	
	const handleBccBtn2Click = () => {
	  setIsMailBccVisible(true);
	  setIsMailCcVisible(false);
	};
	
	const handleCcBtn2Click = () => {
	  setIsMailCcVisible(true);
	  setIsMailBccVisible(false);
	};
	...
	return (
		<div>
                          ...
                          Prev
									...
									<div className= "hs-tooltip ti-main-tooltip">
										<Link to="#" className= "hs-tooltip-toggle text-lg leading-none text-gray-500 dark:text-white/70 flex">
											<i className= "ri-arrow-right-s-line rtl:rotate-0"></i>
											<span className= "hs-tooltip-content ti-main-tooltip-content py-1 px-2 bg-gray-900 text-xs font-medium text-white shadow-sm dark:bg-slate-700" role="tooltip" data-popper-placement="top" style={{position: "fixed", inset: "auto auto 0px 0px", margin: "0px", transform: "translate(1771px, -358px)"}}>
                          Next
											</span>
										</Link>
			...
			<div id="mail-compose" className={`hs-overlay ${isMailCcVisible || isMailBccVisible ? '' : 'hidden'} ti-modal ${ModelClass}`}>
						...
						<div className= "ti-modal-body">
							<div className= "space-y-3">
								<div className= "flex rounded-sm shadow-sm mail-to">
									...
									<div onClick={handleCcBtnClick} className= "btn-cc px-4 inline-flex items-center min-w-fit border ltr:border-l-0 rtl:border-r-0 border-gray-200 bg-gray-50 dark:bg-black/20 dark:border-white/10">
										<span className= "text-sm text-gray-500 dark:text-white/70">CC</span>
									</div>
									<div onClick={handleBccBtnClick} className= "btn-bcc px-4 inline-flex items-center min-w-fit  ltr:rounded-r-sm rtl:rounded-l-sm border ltr:border-l-0 rtl:border-r-0 border-gray-200 bg-gray-50 dark:bg-black/20 dark:border-white/10">
										<span className= "text-sm text-gray-500 dark:text-white/70">BCC</span>
									</div>
								</div>
								<div className={`flex rounded-sm shadow-sm ${isMailCcVisible ? 'mail-cc' : 'hidden'} `}>
									<input type="email" className= "ti-form-input  ltr:rounded-r-none rtl:rounded-l-none focus:z-10" placeholder="CC" required/>
									<div onClick={handleCcBtn2Click} className= "btn-cc-bcc px-4 inline-flex items-center min-w-fit  ltr:rounded-r-sm rtl:rounded-l-sm border ltr:border-l-0 rtl:border-r-0 border-gray-200 bg-gray-50 dark:bg-black/20 dark:border-white/10">
										<span className= "text-sm text-gray-500 dark:text-white/70">BCC</span>
									</div>
								</div>
								<div className={`flex rounded-sm shadow-sm ${isMailBccVisible ? 'mail-bcc' : 'hidden'} `}>
									<input type="email" className= "ti-form-input  ltr:rounded-r-none rtl:rounded-l-none focus:z-10" placeholder="Bcc" required/>
									<div onClick={handleBccBtn2Click} className= "btn-bcc-cc px-4 inline-flex items-center min-w-fit  ltr:rounded-r-sm rtl:rounded-l-sm border ltr:border-l-0 rtl:border-r-0 border-gray-200 bg-gray-50 dark:bg-black/20 dark:border-white/10">
										<span className= "text-sm text-gray-500 dark:text-white/70">Cc</span>
									</div>
								</div>
								<input type="text" id="input-label" className= "ti-form-input focus-visible:outline-none" placeholder="Subject"/>
								<textarea className= "ti-form-input" rows={10} placeholder="Type Your Message Here ......."/>
							</div>
						</div>
