...
import { useTableMenu } from "@/features/sistema/menus/hooks";
import {
  AiOutlineCheck,
  AiOutlineClose,
  AiOutlineEdit,
...
const TableMenu = () => {
  const { deleteMenus, loading, open, search, searchMenu, setSearch } =
    useTableMenu();
  ...
  const currentMenu = searchMenu().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchMenu().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              menu.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteMenus(menu.id, menu.estado)}
                          >
                            {menu.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchMenu().length)} de{" "}
              {searchMenu().length} registros
            </p>
          </div>
        </div>
      </div>
