...
import { useTableOpciones } from "@/features/sistema/opciones/hooks";
import {
  AiOutlineCheck,
  AiOutlineClose,
  AiOutlineEdit,
...
const TableOpciones = () => {
  const { deleteOpciones, loading, open, search, searchOpcion, setSearch } =
    useTableOpciones();
  ...
  const currentOpcion = searchOpcion().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchOpcion().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              opcion.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteOpciones(opcion.id, opcion.estado)}
                          >
                            {opcion.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchOpcion().length)} de{" "}
              {searchOpcion().length} registros
            </p>
          </div>
        </div>
      </div>
