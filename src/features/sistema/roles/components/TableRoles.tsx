...
import { useTableRoles } from "@/features/sistema/roles/hooks";
import {
  AiOutlineCheck,
  AiOutlineClose,
  AiOutlineEdit,
...
const TableRoles = () => {
  ...
  const currentRol = searchRol().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchRol().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                  ...
                  {currentRol.map((rol) => (
                    <tr key={rol.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        >
                          {rol.estado ? <AiOutlineCheck /> : <AiOutlineClose />}
                        </span>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className="p-2 inline-flex items-center gap-2 font-medium rounded-md text-gray-500"
                            type="button"
                            data-hs-overlay="#hs-focus-management-modal-rol-permiso"
                            onClick={() => handleOpenRolPermisoModal(rol)}
                          >
                            <AiOutlineSetting />
                          </button>
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              rol.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteRoles(rol.id, rol.estado)}
                          >
                            {rol.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchRol().length)} de{" "}
              {searchRol().length} registros
            </p>
          </div>
        </div>
      </div>
