import { useModalRoles } from "@/features/sistema/roles/hooks";
import { Controller } from "react-hook-form";
import { Loader } from "@/components";

const ModalRoles = () => {
  const { currentRoles, handleClose, loading, methodsRoles, onSubmit } =
    useModalRoles();

  return (
    <div
      id="hs-focus-management-modal-rol"
        ...
        <div className="ti-modal-content">
          <div className="ti-modal-header">
            <h3 className="ti-modal-title">
              {currentRoles.id ? "Editar" : "Agregar"} rol
            </h3>
            <button
              type="button"
          ...
          <form onSubmit={methodsRoles.handleSubmit(onSubmit)}>
            <div className="ti-modal-body">
              <Controller
                name="nombre"
                control={methodsRoles.control}
                rules={{ required: true }}
                defaultValue=""
                render={({ field }) => (
                  <div className="mt-2">
                    <label
                      className={`block text-sm mb-2 dark:text-white ${
                        methodsRoles.formState.errors.nombre
                          ? "text-red-500"
                          : ""
                      }`}
                    >
                    ...
                    />
                    {methodsRoles.formState.errors.nombre ? (
                      methodsRoles.formState.errors.nombre.type ===
                      "required" ? (
              ...
              <Controller
                ...
                render={({ field }) => (
                  <div className="mt-2">
                    <label
                      className={`block text-sm mb-2 dark:text-white ${
                        methodsRoles.formState.errors.descripcion
                          ? "text-red-500"
                          : ""
                      }`}
                    >
                    ...
                    />
                    {methodsRoles.formState.errors.descripcion ? (
                      methodsRoles.formState.errors.descripcion.type ===
                      "required" ? (
                        <span className="text-xs text-red-500">
