import { useTablePais } from "@/features/registro/paises/hooks";
...
const TablePais = () => {
  const { deletePais, search, searchPais, setSearch, handleCurrent, loading } =
    useTablePais();
  ...
  const currentPaises = searchPais().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchPais().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            onClick={() => handleCurrent(pais)}
                          >
                            <AiOutlineEdit />
                          </button>
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              pais.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deletePais(pais.id, pais.estado)}
                          >
                            {pais.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchPais().length)} de{" "}
              {searchPais().length} registros
            </p>
          </div>
        </div>
      </div>
