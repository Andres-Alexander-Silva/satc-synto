...
import { useTableVehiculos } from "@/features/registro/vehiculos/hooks";
import { LoaderComponents } from "@/components";

const columns = [
  { id: "placa", label: "Placa" },
  { id: "marca", label: "Marca" },
  { id: "tipoVehiculo", label: "Tipo vehiculo" },
  { id: "pais", label: "Pais" },
  { id: "modelo", label: "Modelo" },
  { id: "numChasis", label: "Numero chasis" },
  { id: "numHabilitacion", label: "Numero habilitacion" },
...
const TableVehiculos = () => {
  const { deleteVehiculos, loading, open, search, searchVehiculo, setSearch } =
    useTableVehiculos();
  ...
  const currentVehiculo = searchVehiculo().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchVehiculo().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              deleteVehiculos(vehiculo.id, vehiculo.estado)
                            }
                          >
                            {vehiculo.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchVehiculo().length)} de{" "}
              {searchVehiculo().length} registros
            </p>
          </div>
        </div>
      </div>
