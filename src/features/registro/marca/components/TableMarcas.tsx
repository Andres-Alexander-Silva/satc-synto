...
import { useTableMarcas } from "@/features/registro/marca/hooks";
...
const TableMarcas = () => {
  const { deleteMarcas, loading, open, search, searchMarca, setSearch } =
    useTableMarcas();
  ...
  const currentMarca = searchMarca().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchMarca().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              marca.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteMarcas(marca.id, marca.estado)}
                          >
                            {marca.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchMarca().length)} de{" "}
              {searchMarca().length} registros
            </p>
          </div>
        </div>
      </div>
