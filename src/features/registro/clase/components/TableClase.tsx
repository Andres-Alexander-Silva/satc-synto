...
import { useTableClase } from "@/features/registro/clase/hooks";
...
const TableClase = () => {
  const { deleteClase, loading, open, search, searchClase, setSearch } =
    useTableClase();
  ...
  const currentClase = searchClase().slice(indexOfFirstItem, indexOfLastItem);

  const totalPages = Math.ceil(searchClase().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              clase.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteClase(clase.id, clase.estado)}
                          >
                            {clase.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchClase().length)} de{" "}
              {searchClase().length} registros
            </p>
          </div>
        </div>
      </div>
