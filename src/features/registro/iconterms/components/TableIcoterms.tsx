...
import { useTableIcoterms } from "@/features/registro/iconterms/hooks";
...
const TableIcoterms = () => {
  const { deleteIcoterms, loading, open, search, searchIcoterm, setSearch } =
    useTableIcoterms();
  ...
  const currentIconterm = searchIcoterm().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchIcoterm().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              deleteIcoterms(iconterm.id, iconterm.estado)
                            }
                          >
                            {iconterm.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchIcoterm().length)} de{" "}
              {searchIcoterm().length} registros
            </p>
          </div>
        </div>
      </div>
