...
import { useModalIcoterms } from "@/features/registro/iconterms/hooks";
import { Loader } from "@/components";

const ModalIcoterms = () => {
  const { currentIcoterms, loading, methodsIcoterms, onSubmit, handleClose } =
    useModalIcoterms();

  return (
    <div
      id="hs-focus-management-modal-iconterm"
        ...
        <div className="ti-modal-content">
          <div className="ti-modal-header">
            <h3 className="ti-modal-title">
              {currentIcoterms.id ? "Editar" : "Agregar"} iconterms
            </h3>
            <button
              type="button"
          ...
          <form onSubmit={methodsIcoterms.handleSubmit(onSubmit)}>
            <div className="ti-modal-body">
              <Controller
                name="codigo"
                control={methodsIcoterms.control}
                rules={{
                  required: true,
                }}
                defaultValue=""
                  ...
                  <div>
                    <label
                      className={`block text-sm mb-2 dark:text-white ${
                        methodsIcoterms.formState.errors.codigo
                          ? "text-red-500"
                          : ""
                      }`}
                    >
              ...
              <Controller
                ...
                render={({ field }) => (
                  <div>
                    <label
                      className={`block text-sm mb-2 dark:text-white ${
                        methodsIcoterms.formState.errors.descripcion
                          ? "text-red-500"
                          : ""
                      }`}
                    >
