...
import { useTableTipoVehiculos } from "@/features/registro/tipo_vehiculo/hooks";
...
const TableTipoVehiculos = () => {
  ...
  const currentTipoVehiculo = searchTipoVehiculo().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchTipoVehiculo().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
          <div className="py-2 px-3">
            <div className="relative max-w-xs">
              ...
              <input
                ...
                placeholder="Buscar tipo vehiculos"
                value={search}
            ...
            {loading ? (
              ...
              <table className="ti-custom-table ti-custom-table-head">
                ...
                <tbody className="bg-white divide-y divide-gray-200 dark:bg-bgdark dark:divide-gray-700">
                  {currentTipoVehiculo.map((vehiculo) => (
                    <tr key={vehiculo.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              deleteTiposVehiculos(vehiculo.id, vehiculo.estado)
                            }
                          >
                            {vehiculo.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchTipoVehiculo().length)} de{" "}
              {searchTipoVehiculo().length} registros
            </p>
          </div>
        </div>
      </div>
...
export default TableTipoVehiculos;
