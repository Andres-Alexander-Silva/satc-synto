...
import { useTablePuntoSalida } from "@/features/registro/punto_salida/hooks";
...
const TablePuntoSalida = () => {
  ...
  const currentPuntoSalida = searchPuntoSalida().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchPuntoSalida().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              ps.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deletePuntosSalida(ps.id, ps.estado)}
                          >
                            {ps.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchPuntoSalida().length)} de{" "}
              {searchPuntoSalida().length} registros
            </p>
          </div>
        </div>
      </div>
...
export default TablePuntoSalida;
