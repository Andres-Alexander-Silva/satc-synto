import { useTableMunicipios } from "@/features/registro/municipios/hooks";
...
const TableMunicipios = () => {
  ...
  const currentMunicipio = searchMunicipios().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchMunicipios().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                  ...
                  {currentMunicipio.map((mun) => (
                    <tr key={mun.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        >
                          {mun.estado ? <AiOutlineCheck /> : <AiOutlineClose />}
                        </span>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              mun.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() => deleteMunicipios(mun.id, mun.estado)}
                          >
                            {mun.estado ? (
                              <AiOutlineUnlock />
                            ) : (
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchMunicipios().length)} de{" "}
              {searchMunicipios().length} registros
            </p>
          </div>
        </div>
      </div>
