...
import { useTableUsuarios } from "@/features/configuracion/usuarios/hooks";
...
const columns = [
  ...
  { id: "ultAcceso", label: "Ultimo acceso" },
...
const TableUsuarios = () => {
  const { deleteUsuarios, loading, open, search, searchUsuario, setSearch } =
    useTableUsuarios();
  ...
  const currentUsuario = searchUsuario().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchUsuario().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                  ...
                  {currentUsuario.map((usuario) => (
                    <tr key={usuario.id}>
                      <td className="px-4 py-3 whitespace-nowrap dark:text-white/70">
                        {usuario.datospersonales.noDocumento}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap dark:text-white/70">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              deleteUsuarios(usuario.id, usuario.estado)
                            }
                          >
                            {usuario.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchUsuario().length)} de{" "}
              {searchUsuario().length} registros
            </p>
          </div>
        </div>
      </div>
