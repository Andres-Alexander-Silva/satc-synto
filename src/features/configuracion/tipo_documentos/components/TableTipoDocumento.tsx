...
import { useTableTipoDocumento } from "@/features/configuracion/tipo_documentos/hooks";
...
const TableTipoDocumento = () => {
  ...
  const currentTipoDocumento = searchTipoDoc().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchTipoDoc().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                  ...
                  {currentTipoDocumento.map((doc) => (
                    <tr key={doc.id}>
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        ...
                        >
                          {doc.estado ? <AiOutlineCheck /> : <AiOutlineClose />}
                        </span>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              doc.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() =>
                              deleteTipoDocumento(doc.id, doc.estado)
                            }
                          >
                            {doc.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchTipoDoc().length)} de{" "}
              {searchTipoDoc().length} registros
            </p>
          </div>
        </div>
      </div>
