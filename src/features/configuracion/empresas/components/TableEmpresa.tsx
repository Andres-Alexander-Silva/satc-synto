...
import { useTableEmpresa } from "@/features/configuracion/empresas/hooks";
import {
  AiOutlineCheck,
  AiOutlineClose,
  AiOutlineEdit,
...
const columns = [
  { id: "identificacion", label: "Idendificacion" },
  { id: "razon_social", label: "Razon social" },
  { id: "direccion", label: "Direccion" },
  { id: "telefono", label: "Telefono" },
  { id: "correo_electronico", label: "Correo electronico" },
  { id: "estado", label: "Estado" },
  { id: "acciones", label: "Acciones" },
];

const TableEmpresa = () => {
  const { deleteEmpresas, loading, open, search, setSearch, searchEmpresa } =
    useTableEmpresa();
  ...
  const currentEmpresa = searchEmpresa().slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(searchEmpresa().length / itemsPerPage);
        ...
        <div className="table-bordered rounded-md ti-custom-table-head">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          <button
                            ...
                            onClick={() => open(empresa)}
                          >
                            <AiOutlineEdit />
                          </button>
                          <button
                            className={`p-2 inline-flex items-center gap-2 font-medium rounded-md ${
                              empresa.estado ? "text-green-500" : "text-red-500"
                            }`}
                            type="button"
                            onClick={() =>
                              deleteEmpresas(empresa.id, empresa.estado)
                            }
                          >
                            {empresa.estado ? (
                              <AiOutlineUnlock />
          ...
          <div className="flex justify-end pr-4 pb-2">
            <p className="text-gray-500 text-sm dark:text-white/70">
              ...
              {Math.min(indexOfLastItem, searchEmpresa().length)} de{" "}
              {searchEmpresa().length} registros
            </p>
          </div>
        </div>
      </div>
